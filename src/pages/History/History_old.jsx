...
const History = () => {
	...
	const [educations, setEducations] = useState([]); //학력, educationList
	const [employments, setEmployments] = useState([]); //경력
	const [activities, setActivities] = useState([]); //활동 및 경험(대외활동&동아리)
	const [projects, setProjects] = useState([]); //프로젝트 (프로젝트&대회)
	const [eduCareers, setEduCareers] = useState([]); //내 커리어의 교육
	const [licenses, setLicenses] = useState([]); //자격증 및 외국어
	const [awards, setAwards] = useState([]); //수상
	const [skills, setSkills] = useState([]); //스킬
	const [files, setFiles] = useState([]); //추가 자료
