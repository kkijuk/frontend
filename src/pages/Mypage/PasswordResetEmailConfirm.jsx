...
const EmailVerificationForm = ({ handleNextStep, handleResendCode, handlePrevStep }) => {
	...
	return (
		<FormContainer>
			<div className="prev-button" onClick={handlePrevStep}>
				{'<'}
			</div>
			<h2>비밀번호 재설정</h2>
			<Instructions>
				이메일로 비밀번호 재설정을 위한 인증번호를 발송했습니다.
				<br />
			...
			<TimerAndResendContainer>
				...
				{timeExpired && <ResendButton onClick={handleResend}>다시 보내기</ResendButton>}
			</TimerAndResendContainer>
			<SubmitButton onClick={handleConfirmClick}>확인</SubmitButton>
			{errorMessage && <ErrorMessage>{errorMessage}</ErrorMessage>}
			<HelpText>
				이메일이 확인되지 않는다면 스팸 메일함을 확인해주시고,
				<br />
				여전히 문제가 발생한다면 끼적에게 문의해주세요.
				<br />
				kkijuk30@gmail.com
			</HelpText>
		</FormContainer>
