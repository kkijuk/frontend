...
const SearchBar = ({
    ...
    const safeval = typeof val === 'string' ? val : ''; // value가 문자열이 아닐 경우 대비
    ...
    useEffect(() => {
        ...
        const trimmed= normalizeKeyword(safeval);
        if (trimmed) {
    ...
    const handleKeyPress = (e) => {
        if (e.key === 'Enter' && val.trim()) {
            ...
            onDebounceSearch(normalizeKeyword(safeval)); // 즉시 검색 호출
        }   
    }

    return (
        <SearchBarContainer>
			<SearchIcon onClick={() => safeval && onDebounceSearch(safeval)}>
